...
func storyMode() {
	...
	showTextLn("-- THIS IS STORY MODE --", 5)
	...
	showText("Good luck...", 3)
	showTextLn("You will need it :)", 5)
	showSeparator()
	storyActOne(player)
	showSeparator()
	storyActTwo(player)
	...
	showSeparator()
	showTextLn("-- YOU REACHED THE END OF STORY MODE --", 5)
	showTextLn("Bye, and thank you for playing !", 10)
}

func storyActOne(player assetPlayer) {
	golem := assetMonster{"Golem", 80, 5}
	showTextLn("-- ACT ONE --", 3)
...
func storyActTwo(player assetPlayer) {
	...
	showTextLn("-- ACT TWO --", 3)
	showTextLn("The bleeding did not stop.. You are feeling worse!", 4)
	getHit(&player, golem.Damage+random.Intn(5))
...
func storyActThree(player assetPlayer) {
	...
	showTextLn("-- ACT THREE --", 3)
	showTextLn(("Just outside of the damned forest, you come across a village. You meet a" + witcher.Name), 2)
	showTextLn("He does not seem happy to see you...", 4)
	getHit(&player, witcher.Damage)
	getHit(&player, witcher.Damage+random.Intn(5))
	getHit(&player, witcher.Damage-random.Intn(5))
	showTextLn("Wow, a triple attack?", 3)
	showTextLn("You suck at this game..", 3)
	showTextLn("But it is not your fault... W had less time for the Story Mode :)", 5)
}
